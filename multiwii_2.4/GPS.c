...
//#endif
            ...
            case NAV_STATE_HOLD_TIMED:
                ...
                } else if (nav_hold_time==0) {           //did we reach our time limit ?
...
int32_t get_new_altitude() {        //估算出本次时刻应该到达的高度
    int32_t diff ;
    int8_t _scale ;
    int32_t change;
...
// Nav PID NAV
            ...
            lastNavDiff = navDiff;					
						...
						if (abs(navDiff)>30) NAV_deltaSum = 0; // Filter out too big values  //限制值可以再考虑一下！！！！
...
//    
//}
void CradleControl(int16_t altitude){										//高度差h 单位cm
    ...
    float temp = 0;  //弧度转换
	  ...
	  //2020年4月24日=============================================================
		...
		if(wp_distance<2500){						//向航点飞行，距离小于25米
			wp_distancelast = wp_distance;
			passflag = 1;
		}
		else if(passflag==1 && wp_distance>2500){		//经过航点 目标航点跳转为下一个航点
			passflag = 2;
		}
		if(passflag==2){
			GPS_distance_cm(&GPS_coord[LAT],&GPS_coord[LON],&GPS_FROM[LAT],&GPS_FROM[LON],&wp_distancelast);
