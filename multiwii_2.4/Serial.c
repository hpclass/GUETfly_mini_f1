...
volatile uint16_t serialHeadRX[UART_NUMBER],serialTailRX[UART_NUMBER];
...
// *******************************************************
#if defined(TEENSY20)
unsigned char T_USB_Available() { //查询缓存
    int n = GetEPRxCount(ENDP2);
    if (n > 255) n = 255;
    return n;
}
#endif
...
#if defined(PROMINI) || defined(MEGA)//配置中断，发送,
#if defined(PROMINI)
// ISR(USART_UDRE_vect) {  // Serial 0 on a PROMINI  //判断寄存器空
#endif
#if defined(MEGA)
    ...
    void UartSendData(uint8_t port) {
        ...
        case 2:
            while(serialHeadTX[2] != serialTailTX[2]) {
                ...
                USART2->DR = (u8) serialBufferTX[2][serialTailTX[2]];
            }
            break;
        case 3:
            while(serialHeadTX[3] != serialTailTX[3]) {
                ...
                USART3->DR = (u8) serialBufferTX[3][serialTailTX[3]];
            }
            break;
...
// we don't care about ring buffer overflow (head->tail) to avoid a test condition : data is lost anyway if it happens
    void store_uart_in_buf(uint8_t data, uint8_t portnum) {
        ...
        uint16_t h = serialHeadRX[portnum];
        serialBufferRX[portnum][h++] = data;
...
//  ISR(USART_RX_vect)  { store_uart_in_buf(UDR0, 0); }
    ...
    uint8_t SerialRead(uint8_t port) {
        ...
        */
        uint16_t t = serialTailRX[port];
        //uint8_t t = serialHeadRX[port];
        uint16_t c = serialBufferRX[port][t];
    ...
    uint8_t SerialAvailable(uint8_t port) {//stm32 add
        return ((uint8_t)( ABS(serialTailRX[port] - serialHeadRX[port]) ))%RX_BUFFER_SIZE;//差值求余
    }

    uint8_t SerialUsedTXBuff(uint8_t port) {
        return ((uint8_t)(ABS(serialTailTX[port] - serialHeadTX[port]) ))%TX_BUFFER_SIZE;
    }
