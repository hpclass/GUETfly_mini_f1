...
//    servo[0] += get_middle(0);
    ...
    // motor[1] = rcCommand[PITCH]+get_middle(1);
    //		servo[0] = rcCommand[PITCH] + get_middle(0);
    servo[5] = -servo5;
    servo[0] = get_middle(0) - servo0;
    // servo0 = 0;
    // servo5 = 0;
    // motor[1] =rcCommand[PITCH];// + get_middle(1);
    for (i = 3; i < 7; i++)
...
//		debug[3] = servo[5];   //横滚
    ...
    else
    {
        static uint8_t d = 0;
        if (!(++d % conf.governorD))
            governorThrottle -= 10; // decay; signal stepsize 10 should  be smooth on most ESCs
    }
    if (governorThrottle < 0)
        governorThrottle = 0; // always beware of negative impact of governor on throttle
#endif

    if (f.PASSTHRU_MODE)
    { // Use Rcdata Without sensors
...
#error "VOLTAGEDROP_COMPENSATION requires correction values which fit VBATNOMINAL; not yet defined for your value of VBATNOMINAL"
...
#if (LOG_VALUES >= 3) || defined(POWERMETER_SOFT)
    {
        ...
        { // by all means - must avoid division by zero
            ampsum = 0;
            for (i = 0; i < NUMBER_MOTOR; i++)
            {
