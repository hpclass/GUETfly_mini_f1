...
{
    int32_t BaroAlt;
    static int32_t BaroAlt_last;
    static int32_t Sonar_alt_u_last;
    static int16_t error_B, error_S, error_select;
    int16_t error16;
    ...
    if (calibratingB > 0)
    {
        logBaroGroundPressureSum = log(baroPressureSum * 1.0);
    ...
    BaroAlt = (logBaroGroundPressureSum - log(baroPressureSum * 1.0)) * baroGroundTemperatureScale;
    ...
    applyDeadband(error16, 5); // remove small P parametr to reduce noise near zero position
    ...
    vel = vel * 0.5f + baroVel * 0.5f;
    // D
    alt.vario = vel;

    applyDeadband(alt.vario, 2);

    BaroPID -= constrain(conf.pid[PIDALT].D8 * alt.vario >> 4, -280, 280); // 定高阀值PID
