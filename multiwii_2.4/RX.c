...
void configureReceiver()
{
    ...
    /********超声波加入********************/
    // #if defined(TrigPin)//在定义了管脚之后才应用
    //  #if defined(TrigPin)
    //    pinMode(TrigPin, OUTPUT);  //#define TrigPin 4    #define EchoPin 8
    //   pinMode(EchoPin, INPUT);
    //   digitalWrite(EchoPin, LOW);
    //   PCICR  |= (1<<PCIE1);    //引脚(Port C)变化中断控制寄存器,使能中断向量
    //  PCMSK1 = (1<<PCINT9);  //A2(PC2)作为回波输入,引脚变化屏蔽寄存器,决定哪些引脚触发中断
    // #endif
    // #endif
    /*
...
void readSerial_RX()
{
    while (SerialAvailable(RX_SERIAL_PORT))
    {
        ...
        if (sbusIndex == 25)
        {
            ...
            {
                if (failsafeCnt > 20)
                    failsafeCnt -= 20; // clear FailSafe counter
                else
                    failsafeCnt = 0;
            }
#endif
...
#if !defined(SBUS__)
#ifdef PPM_AT9
#define PPM_ADD_TIME 505
#endif

#ifdef PPM_FSI6
#define PPM_ADD_TIME 403
#endif
