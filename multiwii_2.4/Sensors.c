...
void EX_I2C_Start(void)
{
    ...
    EX_SCL_H;
    EX_Delay_1us(1);
    if(!EX_SDA_read) return;//SDA线为低电平则总线忙,退出
    EX_SDA_L;
    EX_Delay_1us(1);
    EX_SCL_L;
    EX_Delay_1us(1);
...
uint8_t Baro_update() {                          // first UT conversion is started in init procedure
    if (_ms561101ba_ctx.state == 2) {               // a third state is introduced here to isolate calculate() and smooth timecycle spike
        _ms561101ba_ctx.state = 0;
        i2c_MS561101BA_Calculate();
        return 1;
    }
    if ((int16_t)(currentTime - _ms561101ba_ctx.deadline)<0) return 0; // the initial timer is not initialized, but in any case, no more than 65ms to wait.
