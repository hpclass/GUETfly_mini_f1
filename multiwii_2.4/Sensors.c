...
void i2c_read_reg_to_buf(uint8_t add, uint8_t reg, uint8_t buf[6], uint8_t size) {
    ...
    *b = i2c_readNak();
    ...
    u8 i=0;
    if(!(*(buf+i)=Single_ReadI2C(add<<1,reg+i)))
    {
    	i2c_errors_count++;
    	return ;
    }
    ...
    for(i=1;i<size;i++)
    {
    *(buf+i)=Single_ReadI2C((add<<1)|1,reg+i);
    }
    */
...
void GYRO_Common() {
    ...
    temp  = ((imu.gyroADC[PITCH] - imu.gyroADC[ROLL] )*7)/10;
...
#if defined(SENSORS_TILT_45DEG_LEFT)
    temp = ((imu.accADC[PITCH] - imu.accADC[ROLL] )*7)/10;
...
#if defined(SENSORS_TILT_45DEG_LEFT)
    temp = ((imu.magADC[PITCH] - imu.magADC[ROLL] )*7)/10;
...
static void getADC() {
    i2c_getSixRawADC(MAG_ADDRESS,MAG_DATA_REGISTER);
//  MAG_ORIENTATION( ((rawADC[0]<<8) | rawADC[1]) ,//莫名其妙的卡死在这里
//                   ((rawADC[4]<<8) | rawADC[5]) ,
//                   ((rawADC[2]<<8) | rawADC[3]) );
//
...
void Mag_init() {
    ...
    delay(100);
    i2c_writeReg(MAG_ADDRESS,0x0a,0x11);  //Start the first conversion
    delay(100);
}
#if !defined(MPU6050_I2C_AUX_MASTER)
void Device_Mag_getADC() {
