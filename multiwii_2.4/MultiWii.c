...
////////////////////////////
volatile uint16_t sonarTime = 0;  //用于新增超声波测高的时间,us D8接入，中断处理更新该值
//volatile  uint16_t intervalStart=0;  //只需保留16位即可
static uint32_t sonarTimer = 0;

u16 Sonar_alt=0;
u32 Sonar_time_pre=0;
u32 Sonar_time_now=0;
//光流
int16_t  OPTIC_DIS_SUM[2] = {0,0};
int16_t  OPTIC_angle[2] = {0,0};
int16_t  OPTIC_angle2[2] = {0,0};
int16_t  OPTIC_P[2]= {0,0};
int16_t  OPTIC_I[2]= {0,0};
int16_t  OPTIC_ISUM[2]= {0,0};
int16_t  OPTIC_D[2]= {0,0};
int16_t  OPTIC_Pre[2]= {0,0};
static int OPTIC_PID[2][3]= {
    {2,1,1},
    {2,1,1}

};

unsigned long takoff_start_time;
uint8_t sonar_out_of_date;
extern int16_t  sonarAltMy; //平滑后的超声波高度
//初始化在RX.cpp  void configureReceiver()
uint8_t TAKEOFF_COMPLETED=1;//自动起飞
uint8_t TAKEOFF_IN_PROGRESS= 1;
uint8_t TAKEOFF=1;
//================================
/*************超声波更新*****************/
//static int16_t  sonarAltMy;
extern  uint8_t alt_switch;//测距模块标志
extern  int16_t  PressAltMy;//平滑后的气压计高度
extern  int16_t  alt_diff;//气压和超声波的差值
void Sonar_update1() {

    static uint32_t lastsonarTimer = 0;
    static uint32_t nowsonarTimer = 0;
    nowsonarTimer=micros();
    if(nowsonarTimer-lastsonarTimer>17000)
    {
        lastsonarTimer=micros();
        Sonar_time_pre=micros();
    } else {
        return;
    }
    ...
    //sonarAltMy = sonarTime * 0.017;
    //单位是cm  340*100/1000/1000/2 1us的距离 0.017cm   应该小于<23500us为400cm有效距离  //应该对机体倾斜校正
//    sonarAlt = sonarTime * 0.017;  //单位是cm  340*100/1000/1000/2 1us的距离 0.017cm   应该小于<23500us为400cm有效距离  //应该对机体倾斜校正
//    if(ABS(sonarAltMy-sonarAlt)>40)
//    {
//    sonarAlt=sonarAltMy;
//    sonar_out_of_date=1;
//    alt_switch=1;
//    alt_diff=alt.EstAlt-PressAltMy;
//    }else{
//    sonar_out_of_date=0;
//
//    }
//    //sonarAltMy = sonarTime;  //单位是cm  340*100/1000/1000/2 1us的距离 0.017cm   应该小于<23500us为400cm有效距离  //应该对机体倾斜校正
    //debug[2]=sonarAlt;
    // debug[0]++;
}
...
void loop () {
    ...
    if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
        ...
        //debug[3]=alt_diff;			//气压和超声波的差值
        //debug[2]=Sonar_alt;
        //debug[1]=PressAltMy;
        //debug[0]=alt.EstAlt;    //debug[i]对应GUI界面的debug显示的值
        ...
        gps_modes_check = (rcOptions[BOXTAKEOFF]<< 4) + (rcOptions[BOXLAND]<< 3) + (rcOptions[BOXGPSHOME]<< 2) + (rcOptions[BOXGPSHOLD]<<1) + (rcOptions[BOXGPSNAV]);
            ...
            if (f.GPS_FIX) {
                if (GPS_numSat >5 ) {
                    if (prv_gps_modes != gps_modes_check) {                           //Check for change since last loop
                        ...
                        if (rcOptions[BOXGPSHOME]) {                                    // RTH has the priotity over everything else
                        ...
                        } else if (rcOptions[TAKEOFF]) {                               //Land now (It has priority over Navigation)
                            if(!f.TAKEOFF_COMPLETED) {
                                f.GPS_mode = GPS_MODE_HOLD;
                                f.GPS_BARO_MODE = true;
                                GPS_set_next_wp(&GPS_coord[LAT], &GPS_coord[LON],&GPS_coord[LAT], & GPS_coord[LON]);
                                set_new_altitude(alt.EstAlt);
                                NAV_state = NAV_STATE_TAKEOFF_START;
                            }
                            ...
                            f.TAKEOFF_IN_PROGRESS = 0; //自动起飞执行中
        ...
        case 4:
            ...
            //#if SONAR
#if 1
            Sonar_update1(); //debug[2] = sonarAlt;
#endif
#if TrigPin
            if(sonarTimer < currentTime) {
                sonarTimer = currentTime + 40000;
                Sonar_update1();
                //debug[2] = sonarAlt;//debug 超声波
            }
#endif
#ifdef LANDING_LIGHTS_DDR
    ...
    */
    ...
    if (f.BARO_MODE) {
        ...
        if(f.TAKEOFF_IN_PROGRESS)//利用定高实现平稳起飞
        {
            debug[2]++;
            AltHoldCorr += TAKEOFF_SPEED;
            if(ABS(AltHoldCorr) > 512) {
                AltHold += AltHoldCorr/512;
                AltHoldCorr %= 512;
            }
...
//
    /*
        if(f.OPTIC_MODE)//开启光流模式
        {

            OPTIC_angle[0]=OF_DX2;
            OPTIC_angle[1]=OF_DY2;
            OPTIC_angle2[0]=OF_DX2FIX;
            OPTIC_angle2[1]=OF_DY2FIX;
            //OPTIC_DIS_SUM[0]+=OF_DX2FIX;
            //OPTIC_DIS_SUM[1]+=OF_DY2FIX;
            for(axis=0; axis<2; axis++)
            {

                //OPTIC_angle[axis]+=OPTIC_DIS_SUM[axis];
                OPTIC_P[axis]=constrain(OPTIC_angle[axis]*OPTIC_PID[axis][0],-500,+500);


                OPTIC_ISUM[axis]+=constrain(OPTIC_angle2[axis],-50,+50);
                OPTIC_ISUM[axis]=constrain(OPTIC_ISUM[axis],-1000,+1000);
                OPTIC_I[axis]=constrain((OPTIC_ISUM[axis]*OPTIC_PID[axis][1])>>5,-500,+500);

                OPTIC_D[axis]=constrain((OPTIC_angle[axis]-OPTIC_Pre[axis])*OPTIC_PID[axis][2],-500,+500);
                OPTIC_Pre[axis]=OPTIC_angle[axis];

                OPTIC_angle[axis]=constrain(OPTIC_P[axis]+OPTIC_I[axis]-OPTIC_D[axis],-100,+100);
            }


            //debug[2]=OPTIC_angle[0];
            //debug[3]=OPTIC_angle[1];
        }
    */
#if defined(THROTTLE_ANGLE_CORRECTION)
    if(f.ANGLE_MODE || f.HORIZON_MODE) {
    ...
    for(axis=0; axis<2; axis++) {
        ...
        if (f.ANGLE_MODE || f.HORIZON_MODE) { // axis relying on ACC
            ...
            errorAngle         = constrain(rc + GPS_angle[axis],-500,+500) - att.angle[axis] + conf.angleTrim[axis] + OPTIC_angle[axis]; //16 bits is ok here
...
void Sonar_init_my()
{
    ...
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising_Falling; //设置中断触发方式为下降沿触发
...
void EXTI4_IRQHandler()
{
    ...
    if(PBin(4)==0)
    {   //下降沿
        Sonar_time_now=micros();
        Sonar_alt=(Sonar_time_now-Sonar_time_pre)*0.017;
    } else {
        //上升沿
        Sonar_time_pre=micros();
    }
...
int main()
{
    ...
    SysTick_Config(FCLK/OSFREQ);//系统滴答时钟
    USB_CH341_Init();
#if !defined(SBUS__)
...
#if !defined(SBUS__)
    USART3_Configuration(115200);//串口3
#else
    USART3_Configuration(100000);
#endif
    JTAG_Set(SWD_ENABLE);
//    Sonar_init_my();
//    TIM3_PWM_Init(1999,72); //400Hz (0-2000)
//  	TIM4_PWM_Init(1999,720-1);//50hz  720分频，计数2000，，=>10us计数，10us*2000=20ms
