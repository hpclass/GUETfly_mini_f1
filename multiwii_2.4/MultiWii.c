...
uint16_t GPS_speed_smooth[GPS_USER_SMOOTH_LEN]; // GPS速度滤波窗口
    ...
    { // check settings integrity
        ...
        { // check current setting integrity
            if (flashsum != global_conf.flashsum)
                update_constants(); // update constants if firmware is changed and integrity is OK
        }
#else
...
#if defined(POWERMETER)
    for (uint8_t j = 0; j <= PMOTOR_SUM; j++)
        pMeter[j] = 0;
#endif
        /************************************/
#if GPS
...
void go_disarm()
{
    if (f.ARMED)
    {
        ...
        if (failsafeEvents)
            plog.failsafe++; // #acitve failsafe @ disarm
        if (i2c_errors_count > 10)
            plog.i2c++;   // #i2c errs @ disarm
...
void loop()
{
    ...
    { // 50Hz
                ...
                { // Inflight ACC calibration START/STOP
                    ...
                    else
                    {
                        ...
                        if (AccInflightCalibrationArmed)
                            ...
                            SET_ALARM_BUZZER(ALRM_FAC_TOGGLE, ALRM_LVL_TOGGLE_ELSE);
#endif
                    }
                }
#endif
...
#if defined(INFLIGHT_ACC_CALIBRATION)
        if (AccInflightCalibrationArmed && f.ARMED && rcData[THROTTLE] > MINCHECK && !rcOptions[BOXARM])
        ...
        if (last_dorp_status != rcOptions[BOXDROP]) // 如果没有动投弹开关，将会保持投弹状态
        {
        ...
        { // Check GPS status and armed
            ...
            if (f.GPS_FIX)
            {
                if (GPS_numSat > 5)
                {
                    ...
                    { // Check for change since last loop f.FAILSAFE_RTH_ENABLE变为1时，已经调用init_RTH()
...
//            }
...
#if GPS
            if (GPS_Compute() != 0)
                break; // performs computation on new frame only if present
#if defined(I2C_GPS)
            if (GPS_NewData() != 0)
                break; // 160 us with no new data / much more with new data
#endif
#endif
#ifdef LANDING_LIGHTS_DDR
...
#endif
    if (user_mission_dorp.drop_status == 3) // 状态1正在前往远航，状态2前往投弹，状态3进行投弹
    {
        f.USER_DROP = 1; // 导航任务决定
    }
    if (f.USER_DROP)
    {
        //        servo[0]=GPS_conf.dorp_servor_open;
        //        servo[1]=GPS_conf.dorp_servor_open;
        // servo[0] = 1050;
        servo[1] = 1050;
    }
    else
    {
        //        servo[0]=GPS_conf.dorp_servor_close;
        //        servo[1]=GPS_conf.dorp_servor_close;
        // servo[0] = 1950;
        servo[1] = 1950;
    }
    //***********************************
    ...
    if (f.BARO_MODE)
    {
        ...
        { // If autoland is in progress then take over and decrease alt slowly  自动降落，通过修改定高实现
