    ...
    if (updateTimer(&altHoldControlTimer, HZ2US(ALT_HOLD_CONTROL_UPDATE_RATE)))
    {
            ...
            f.ARMED)
        {
            ...
            if (f.GPS_BARO_MODE)
            {
                ...
                if (NAV_state == NAV_STATE_LAND_IN_PROGRESS || NAV_state == NAV_STATE_LAND_DETECTED || NAV_state == NAV_STATE_LANDED)
                { //==?????==========
                ...
                else
                {
                    ...
                    { // reduce desired speed if altitude less then SAFE_NAV_ALTITUDE
            ...
            { // here f.BARO_MODE is always true
                ...
                if (takeOffInProgress || // it means that we are going to take off, so it necessary to skip first AH case to avoid jumps near the ground
                    ...
                    (!f.SAFE_ALT_MODE && (abs(throttleDiff) > ALT_HOLD_THROTTLE_NEUTRAL_ZONE)) || (f.SAFE_ALT_MODE && ((throttleDiff > ALT_HOLD_THROTTLE_NEUTRAL_ZONE) || ((throttleDiff < -ALT_HOLD_THROTTLE_NEUTRAL_ZONE) && (alt.EstAlt > SAFE_ALT_DURING_AH))))
#else
                ...
                { // Alt hold activated, hovering state, throttle at middle neutral zone
...
void applyPIDControl(uint16_t dTime, bool isHoveringState)
{
    ...
    varioPIDControl = ((varioError * conf.pid[PIDALT].P8) >> 5) + varioErrorIPart - (((int32_t)ins.accelEF_Filtered[ALT] * conf.pid[PIDALT].D8) >> 6); //- (((int32_t)ins.accelEF_Filtered[ALT] * conf.pid[PIDALT].D8) >> 6);
...
{
    return (abs(alt.vario) < 15) && (varioErrorIPart <= -VARIO_ERROR_I_PART_LAND_BOUND) && (alt.EstAlt < SAFE_NAV_ALTITUDE);
}
