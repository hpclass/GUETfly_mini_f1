...
bool calculateZ_INS() {
    ...
    if (updateTimer(&calculateZ_INSTimer, HZ2US(INS_UPDATE_RATE))) {
        ...
        calculateRAWAltitude();  //===??¦Æ???????? 20ms?===========

        updateAccelEF_Filtered(ALT);  //==????????????????(20ms)?§Ö?ACC(4ms???????????)??????===================
...
bool calculateXY_INS() {
    ...
    if (updateTimer(&calculateXY_INSTimer, HZ2US(INS_UPDATE_RATE))) {
        ...
        if (
            ...
            correctXYStateWithGPS(&dt);   //==????????????¦É?????????????????????????¦Ë??===========================

            updateXYState(&dt); //==????ACC????????????????????????????????????¦Ë??===========================

            saveXYPositionToHistory();  //===?????¦Ì???????????¦Ë????¡ä¦Å????===========
...
void updateAccelEF_Filtered(uint8_t axis) {
    ins.accelEF_Filtered[axis] = ins.accelEF_Sum[axis] / ins.accelEF_Sum_count[axis];  //===rotateAccelToEarthFrame()?§Ú???=
