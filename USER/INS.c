...
bool calculateZ_INS()
{
    ...
    if (updateTimer(&calculateZ_INSTimer, HZ2US(INS_UPDATE_RATE)))
    {
        ...
        calculateRAWAltitude(); //===??味???????? 20ms?===========

        updateAccelEF_Filtered(ALT); //==????????????????(20ms)?械?ACC(4ms???????????)??????===================
...
void correctZStateWithBaro(float *dt)
{
    ...
    bool isAirCushionEffectDetected = (isTakeOffInProgress() || isGroundDetected()) //==????效?=
...
bool calculateXY_INS()
{
    ...
    if (updateTimer(&calculateXY_INSTimer, HZ2US(INS_UPDATE_RATE)))
    {
            ...
            f.ARMED && f.GPS_FIX && (GPS_numSat >= 4) && f.GPS_FIX_HOME)
        {
            ...
            correctXYStateWithGPS(&dt); //==????????????喂?????????????????????????位??===========================

            updateXYState(&dt); //==????ACC????????????????????????????????????位??===========================

            saveXYPositionToHistory(); //===?????渭???????????位????鈥诧拷?????===========
...
{
    ins.accelEF_Filtered[axis] = ins.accelEF_Sum[axis] / ins.accelEF_Sum_count[axis]; //===rotateAccelToEarthFrame()?懈???=
