...
void DJI_Guidance_uart_protocl(uint8_t c)
{
    ...
    else if (step_flag >= 3 && step_flag <= 7)
    {                             // 跳过RES
        CRC16 = CRC16_(CRC16, c); // 计算CRC
        step_flag++;
    }
    ...
    else if (step_flag == DJI_Guidance_head.m_length - 4)
    { // 校验码
        DJI_Guidance_head.m_data_checksum = 0x000000ff & (c << 0);
        step_flag++;
    }
    else if (step_flag == DJI_Guidance_head.m_length - 3)
    { // 校验码
        DJI_Guidance_head.m_data_checksum |= 0x0000ff00 & (c << 8);
        step_flag++;
    }
    else if (step_flag == DJI_Guidance_head.m_length - 2)
    { // 校验码
        DJI_Guidance_head.m_data_checksum |= 0x00ff0000 & (c << 16);
        step_flag++;
    }
    else if (step_flag == DJI_Guidance_head.m_length - 1)
    { // 校验码
